...
// =================================================================
// 3. DB Helper: connectionQueryPromise
// =================================================================
...
/**
 * @description WebSocket 연결을 등록합니다. (message, invite 모두 사용)
 */
function registerConnection(userId, ws) {
    ...
    logger.info(
        `[WS Manager] User ${userId} connected. Total connections: ${connectedClients[userId].length}`
    );
}
...
function unregisterConnection(userId, ws) {
    if (connectedClients[userId]) {
        ...
        logger.info(
            `[WS Manager] User ${userId} disconnected. Remaining: ${
                connectedClients[userId]?.length || 0
            }`
        );
    }
}
 ...
 * @param {object} payload - 브로드캐스트할 데이터(JSON 직렬화 가능 객체)
 */
async function broadcastMessageToRoom(roomId, payload) {
    ...
    try {
        ...
        const results = await connectionQueryPromise(
            connection,
            sqlGetParticipants,
            [roomId]
        );
                ...
                clients.forEach((client) => {
                    ...
                    if (client.readyState === 1) {
                        try {
                            ...
                            logger.error(
                                `[WS Manager] Failed to send to user ${participantId}: ${err.message}`
                            );
                        }
                    }
                });
        ...
        logger.debug(
            `[WS Manager] Broadcast complete for Room ${roomId}. Target IDs: ${participantIds.join(',')}`
        );
    } catch (error) {
        logger.error(
            `[WS Manager ERROR] Failed to broadcast message for Room ${roomId}: ${error.message}`,
            error
        );
    } finally {
        if (connection) {
