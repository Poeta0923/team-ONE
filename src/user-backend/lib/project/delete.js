...
module.exports = {
    /**
     * @description 프로젝트 삭제 및 참여 인원 & 좋아요 삭제 (팀장 권한 확인 포함)
     */
    delete: async (req, res) => {
        ...
        try {
            ...
            const sqlDeleteProject = `DELETE FROM projects WHERE projectId = ?`;
            const sqlDeleteMembers = `DELETE FROM members WHERE projectId = ?`;
            const sqlDeleteLikes = `DELETE FROM \`like\` WHERE projectId = ?`; // like는 예약어일 가능성이 높아 백틱(`) 사용 권장
            ...
            // [4] 최종 응답 전송
            logger.info(`[Delete Project] 프로젝트 ID: ${projectId} 삭제 완료`);
            res.status(200).json({
            ...
            logger.error(`[Delete Error] 프로젝트 삭제 처리 중 오류 발생: ${error.message}`, error);
            
            // 클라이언트에게 오류 메시지 반환 (권한 오류 처리)
            let statusCode = 500;
            let displayMessage = '프로젝트 삭제 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.';

            if (error.message.includes('AUTH_FAILED')) {
                statusCode = 403;
                displayMessage = '프로젝트를 삭제할 권한이 없습니다. 팀장만 삭제할 수 있습니다.';
            } 
