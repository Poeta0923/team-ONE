...
module.exports = {
    ...
    create: async (req, res) => {
        // 트랜잭션 관리를 위해 connection 객체를 try 블록 외부에 선언
        let connection; 
        ...
        try {
            ...
            // [4-1] 프로젝트 정보 저장 (projects 테이블)
            const result1 = await connectionQueryPromise(connection, sqlProjects, projectValues);
            const newProjectId = result1.insertId;

            // [4-2] 참여 멤버 정보 저장 (members 테이블)
            const memberValues = [
                newProjectId,
                userIdFromToken,
            ];
            ...
            // 클라이언트에게 오류 메시지 반환
            res.status(500).json({ message: '프로젝트 생성 처리 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.' });
